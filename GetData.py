...
def Get_Card(date):
    ...
    if(payload['nextAvailableDateMillis'] != None):
    ...
    if(NBA_date == None):
        ...
        buttom = json.load(open('json/Score/NoGameButtom.json','r',encoding='utf-8'))
        if(nextAvailableDate == None):
            responce['body']['contents'][2]['text'] = '無比賽'
        else:
            responce['body']['contents'][2]['text'] = str(nextAvailableDate)[:-6]
            buttom['action']['data'] = 'Get{}'.format(str(nextAvailableDate)[:-6])
            responce['footer']['contents'].append(buttom)
        return responce
